...
def write_tmp_submission_script_ccrt(job_name,
    ...
    IMAGE_NAME = 'delphi'
    SCRIPT_NAME = 'fbii_pyht_tracking/run_delphi.py'
    with open(job_name, "w") as f:
        ...
        f.write(
            "pcocc run --mount {0:} -I {1:} --entry-point -- python {2:} --filename {3:} --ID_state {4:} --sigma_z {5:} --plane {6:} --scan_type {7:} --current {8:}, --chromaticity {9:} --min_value {10:} --max_value {11:} --n_scan_points {12:}\n".format(MOUNT_FOLDER,
                                                                                                                                                                                                                                                              IMAGE_NAME,
                                                                                                                                                                                                                                                              SCRIPT_NAME,
                                                                                                                                                                                                                                                              filename,
                                                                                                                                                                                                                                                              ID_state,
...
def write_submission_script_slurm(job_name,
    ...
    with open(job_name, "w") as f:
        ...
        f.write(
            "singularity exec --no-home -B {0:} {1:} python {2:}  --filename {3:} --ID_state {4:} --sigma_z {5:} --plane {6:} --scan_type {7:} --current {8:} --chromaticity {9:} --min_value {10:} --max_value {11:} --n_scan_points {12:}\n".format(MOUNT_FOLDER,
                                                                                                                                                                                            IMAGE_NAME,
                                                                                                                                                                                            SCRIPT_NAME,
                                                                                                                                                                                            filename,
                                                                                                                                                                                            ID_state,
