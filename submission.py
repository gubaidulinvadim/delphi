...
def write_tmp_submission_script_ccrt(job_name,
    ...
    with open(job_name, "w") as f:
        ...
        f.write(
            "pcocc run --mount {0:} -I {1:} --entry-point -- python {2:} --filename {3:} --ID_state {4:} --sigma_z {5:} --plane {6:} --scan_type {7:} --current {8:} --chromaticity {9:} --min_value {10:} --max_value {11:} --n_scan_points {12:} --Q_s {13:}\n".format(MOUNT_FOLDER,
                                                                                                                                                                                                                                                                         IMAGE_NAME,
                                                                                                                                                                                                                                                                         SCRIPT_NAME,
                                                                                                                                                                                                                                                                         filename,
                                                                                                                                                                                                                                                                         ID_state,
...
def write_submission_script_slurm(job_name,
                                  ...
                                  n_scan_points,
                                  Q_s):
    MOUNT_FOLDER = '/lustre/scratch/sources/physmach/gubaidulin/delphi:/home/dockeruser/delphi'
    IMAGE_NAME = '/lustre/scratch/sources/physmach/gubaidulin/delphi.sif'
    SCRIPT_NAME = '/home/dockeruser/delphi/run_delphi.py'
    with open(job_name, "w") as f:
        f.write("#!/bin/bash\n")
        f.write("#SBATCH --partition metis\n")
        f.write("#SBATCH -n 8\n")
        f.write("#SBATCH --time={:}\n".format(job_time))
        ...
        f.write(
            "singularity exec -e --no-home -B {0:} {1:} python {2:}  --filename {3:} --ID_state {4:} --sigma_z {5:} --plane {6:} --scan_type {7:} --current {8:} --chromaticity {9:} --min_value {10:} --max_value {11:} --n_scan_points {12:} --Q_s {13:}\n".format(MOUNT_FOLDER,
                                                                                                                                                                                                                                                                     IMAGE_NAME,
                                                                                                                                                                                                                                                                     SCRIPT_NAME,
                                                                                                                                                                                                                                                                     filename,
                                                                                                                                                                                                                                                                     ID_state,
